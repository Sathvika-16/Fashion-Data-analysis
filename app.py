...
def main():
    ...
    nav_selection = st.sidebar.radio("Navigate", 
                                     ["Items purchased by each state", "Monthly purchases", "Payment method by each state",
                                      ...
                                      "Heatmap of top purchase items by monthly", "Recommendation for each state"])

    # Display selected content based on navigation
    if nav_selection == "Items purchased by each state":
